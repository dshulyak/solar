...
class BaseGraphDB(object):
    ...
    COLLECTIONS = Enum(
        'Collections',
        'input resource state_data state_log plan_node plan_graph plan_edge events stage_log commit_log'
    )
    DEFAULT_COLLECTION=COLLECTIONS.resource
    RELATION_TYPES = Enum(
        'RelationTypes',
        'input_to_input resource_input plan_to_node plan_to_edge resource_event'
    )
